module Scan
  ...
  class DetectValues
    ...
    def self.default_device_ios
      ...
      if devices and !devices.empty? # make sure it actually exists
    ...
    def self.detect_destination
      if Scan.config[:destinations]
        ...
        UI.important("Using your value '#{Scan.config[:destinations]}' for now")
      ...
      if Scan.project.ios?
        Scan.config[:destinations] = Scan.devices.map { |d| "platform=iOS Simulator,id=#{d.udid}" }
      elsif Scan.project.tvos?
        Scan.config[:destinations] = Scan.devices.map { |d| "platform=tvOS Simulator,id=#{d.udid}" }
      else
        Scan.config[:destinations] = ["platform=OS X"]
      end
    end
  end
end
